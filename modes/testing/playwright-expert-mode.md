...
test.describe('Network handling', () => {
  ...
  test('intercepts and modifies requests', async ({ page }) => {
    ...
    const [request] = await Promise.all([
      page.waitForRequest((req) => req.url().includes('enhanced=true')),
    ]);
...
test.describe('Visual regression', () => {
  ...
  test('dashboard matches snapshot', async ({ page }) => {
    ...
    await page.locator('.timestamp').evaluateAll((els) =>
      els.forEach((el) => (el.textContent = 'XX:XX:XX'))
    );
