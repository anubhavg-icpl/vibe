...
export function renderProgressBar(options: ProgressOptions): string {
  ...
  // Use amber-filled / dim-empty blocks  ▰▱
...
export function renderSpinner(frame: number, message?: string): string {
  const frames = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"];
  const spinner = colors.primary(frames[frame % frames.length]);
...
export function renderInstallProgress(
  ...
  const bar    = renderProgressBar({ total, current, width: 25 });
  const status = `${colors.dim(symbols.arrow)} ${colors.secondary(currentMode)} ${colors.dim("→")} ${colors.accent(currentAgent)}`;
  return `${bar}\n  ${status}`;
}
...
export function renderParallelProgress(
  ...
  for (const task of active.slice(0, 4)) {
    lines.push(`  ${colors.primary("⠿")}  ${colors.secondary(task.mode)} ${colors.dim("→")} ${task.agent}`);
  }
  if (active.length > 4) {
    lines.push(colors.dim(`  … ${active.length - 4} more in progress`));
  }
  ...
  if (errors.length > 0) {
    lines.push("");
    lines.push(colors.error(`  ${symbols.cross} ${errors.length} failed`));
  }
...
export function renderCompletionSummary(
  ...
  if (failed === 0) {
    lines.push(colors.successBold(`${symbols.check}  All ${successful} installations completed`));
...
export class ProgressTracker {
  ...
  getProgress(): { current: number; total: number; elapsed: number } {
    return { current: this.current, total: this.total, elapsed: Date.now() - this.startTime };
  }
