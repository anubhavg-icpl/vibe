...
> VIBE house flagship reference system prompt. Full, advanced assistant prompt
> based on the Claude Fable 5 / Mythos-class system prompt. Kept verbatim for
> study and as the repo's reference "Mythos" prompt. Installable via the VIBE
> CLI: `vibe add mythos -k system-prompt`.
...
> End of reference prompt. Source: Claude Fable 5 / Mythos-class system prompt
> (see also `system-prompts/Anthropic/claude-fable-5.md`). Tool-definition and
> user-context sections from the original are abbreviated here; the canonical
> full leak with every tool schema lives under `system-prompts/Anthropic/`.
